...
func Add(conn string, auth gocb.PasswordAuthenticator, bucketName, bucketPassword string, key string) error {
...
func AddSafe(conn string, auth gocb.PasswordAuthenticator, bucketName, bucketPassword string, key string) error {
	cluster, err := gocb.Connect(conn)
	if err != nil {
		return fmt.Errorf("error connecting to the cluster: %s", err.Error())
	}
